...
function derive(password, salt, iterations) { return crypto.pbkdf2Sync(password, salt, iterations, 64, 'sha512'); }
function hashPassword(password, salt = crypto.randomBytes(16).toString('hex')) {
  return { salt, hash: derive(password, salt, PBKDF2_ITERATIONS).toString('hex'), iterations: PBKDF2_ITERATIONS };
}
function verifyPassword(password, salt, hash, iterations = 100000) {
  if (!password || !salt || !hash) return false;
  const check = derive(password, salt, Number(iterations) || 100000);
...
function save(){if(!cache)return;const tmp=`${DATA_FILE}.tmp`;fs.writeFileSync(tmp,JSON.stringify(cache,null,2),{mode:0o600});fs.renameSync(tmp,DATA_FILE)}
...
module.exports={load,save,nextId,hashPassword,verifyPassword,DATA_FILE,PBKDF2_ITERATIONS};
