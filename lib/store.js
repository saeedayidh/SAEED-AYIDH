...
function load() {
  ...
  const ds = d.settings;
  ['whatsapp','customCss','customJs','mainSiteUrl','helpCenterUrl','servicesTitle','promoBannersTitle','topBar'].forEach(k => {
    if (cache.settings[k] === undefined) cache.settings[k] = ds[k];
  });
  if (!cache.services) cache.services = d.services;
  if (!cache.imageBanners) cache.imageBanners = d.imageBanners;
  if (!cache.promoBanners) cache.promoBanners = d.promoBanners;
  return cache;
}
