...
export const NewsSection: React.FC = () => {
  const { data } = useCMS();
  const section = data.sections.find(s => s.id === 'news');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center text-center space-y-4 max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#191919] border border-white/10 text-xs font-semibold text-[#D51F2B]">
            ...
            <span>{section?.badge || 'المركز الإخباري والتغطيات'}</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight">{section?.title || 'أخبار سعيد'}</h2>
          <p className="text-base sm:text-lg text-[#B8B8B8] font-light leading-relaxed">
            {section?.subtitle || 'آخر الأخبار والتغطيات وإعلانات المشاريع والشراكات الجديدة.'}
          </p>
