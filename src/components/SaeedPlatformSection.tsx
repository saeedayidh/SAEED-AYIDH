...
const rows:[string,string,string,string,string,string,string][]=[
 ['archive','أرشيف سعيد','خط زمني مرتب لأعمال ومشاريع سعيد من البداية إلى اليوم.','الأرشيف','Saeed Archive','A timeline of Saeed’s work and projects.',''],
 ...
 ['stories','قصص سعيد','مكتبة للقصص بتصنيفات وأغلفة وصفحات مستقلة.','القصص','Saeed Stories','A categorized story library with dedicated pages.',''],
 ...
 ['press','الملف الإعلامي','ملف إعلامي جاهز للشركات والجهات.','الإعلام','Press Kit','A ready-to-share media profile.',''],
 ...
 ['numbers','أعمال سعيد بالأرقام','إحصائيات المحتوى والمشاريع والخبرة والتغطيات.','الأرقام','Saeed in Numbers','Content, project and experience statistics.',''],
 ['projects','خريطة مشاريع سعيد','عرض مشاريع سعيد وعلاقة كل مشروع بالهوية الرئيسية.','المشاريع','Project Map','A visual map of Saeed’s projects.',''],
 ...
 ['polls','تصويت الجمهور','تصويت الجمهور على المحتوى القادم والاختيارات.','التصويت','Audience Polls','Vote on upcoming content and choices.',''],
 ['questions','أسئلة سعيد','استقبال أسئلة الجمهور وعرض الإجابات المختارة.','الأسئلة','Saeed Q&A','Collect questions and publish selected answers.',''],
 ['notify','إشعارات سعيد','اختيار أنواع المحتوى التي يريد الزائر متابعتها.','الإشعارات','Saeed Notifications','Choose which content to follow.',''],
 ['whatsnew','ما الجديد؟','عرض آخر إضافات وتحديثات الموقع.','التحديثات','What’s New','See the latest site additions and updates.',''],
 ...
 ['search','البحث الشامل','بحث واحد داخل القصص والأخبار والبلوق والأعمال والأدوات.','البحث','Universal Search','Search stories, news, blog posts, work and tools.',''],
...
export const SaeedPlatformSection:React.FC=()=>{const{data}=useCMS();const{language,isArabic}=useLanguage();const platform:SaeedPlatform=(data.global as any).saeedPlatform||defaultSaeedPlatform;if(!platform.enabled)return null;const items=(platform.items||[]).filter(x=>x.enabled&&!retiredPlatformIds.has(x.id)&&!topUtilityIds.has(x.id)).sort((a,b)=>a.order-b.order);const text=(item:PlatformItem,key:'title'|'description'|'badge')=>language==='en'?((item as any)[`${key}En`]||item[key]):item[key];return <div id="saeed-sections">{items.map((item,index)=><section key={item.id} id={`saeed-${item.id}`} className={`relative overflow-hidden border-t border-white/5 py-20 sm:py-24 ${index%2===0?'bg-[#090909]':'bg-[#0D0D0D]'}`} dir={isArabic?'rtl':'ltr'}><div className="mx-auto max-w-7xl px-5 sm:px-8"><div className={`grid items-center gap-10 ${item.imageUrl||item.videoUrl?'lg:grid-cols-2':'grid-cols-1'}`}><div className="max-w-3xl"><div className="mb-3 text-xs font-black text-[#D51F2B]">{text(item,'badge')}</div><h2 className="text-3xl font-black tracking-tight sm:text-5xl">{text(item,'title')}</h2><p className="mt-5 text-base leading-8 text-gray-400 sm:text-lg">{text(item,'description')}</p><div className="mt-7 flex flex-wrap gap-3">{(item.buttons||[]).filter(b=>b.enabled&&b.label).map((b,i)=><a key={`${item.id}-${i}`} href={b.url&&b.url!=='#'?b.url:`/saeed/${item.id}`} className="inline-flex items-center gap-2 rounded-xl border border-white/15 px-5 py-3 text-xs font-bold transition hover:border-[#D51F2B] hover:text-[#E52E3C]">{language==='en'?(b.labelEn||'Explore'):b.label}<ArrowUpLeft className="h-4 w-4"/></a>)}</div></div>{(item.imageUrl||item.videoUrl)&&<div className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-black shadow-2xl">{item.imageUrl&&<img src={item.imageUrl} alt={text(item,'title')} className="max-h-[480px] w-full object-cover"/>}{item.videoUrl&&<video src={item.videoUrl} controls playsInline preload="metadata" className="max-h-[520px] w-full object-contain"/>}</div>}</div></div></section>)}</div>};
