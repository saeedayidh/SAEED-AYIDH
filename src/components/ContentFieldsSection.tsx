      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#191919] border border-white/10 text-xs font-semibold text-[#D51F2B]">
            ...
            <span>صناعة المحتوى الإبداعي</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight">
            مجالات صناعة المحتوى
          </h2>
          <p className="text-base sm:text-lg text-[#B8B8B8] font-light">
            محتوى متنوع بهوية مختلفة لكل مجال.
          </p>
        </div>

        {/* 4 Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {siteData.contentFields.map((field) => (
            <Link
              key={field.id}
              to={`/content/${field.slug || field.id}`}
              className="sba-card group overflow-hidden flex flex-col justify-between p-6 cursor-pointer"
            >
              {/* Thumbnail Area */}
              <div className="relative h-48 rounded-xl overflow-hidden mb-6 bg-[#080808] border border-white/10">
                ...
                <div className="absolute top-3 right-3 px-3 py-1 rounded-md bg-[#080808]/80 backdrop-blur-md text-[10px] font-bold text-[#D51F2B] border border-white/10">
                  {field.categoryTag}
                </div>
              </div>
              ...
              <div className="space-y-2 mb-6">
                <h3 className="text-xl font-bold text-white group-hover:text-[#D51F2B] transition-colors">
                  {field.title}
                </h3>
              ...
              {/* Arrow CTA */}
              <div className="pt-4 border-t border-white/5 flex items-center justify-between text-xs font-semibold text-gray-400 group-hover:text-white transition-colors">
                <span>استكشف المحتوى الكامل</span>
