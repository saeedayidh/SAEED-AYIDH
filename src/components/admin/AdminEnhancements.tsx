...
export const AdminEnhancements: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  return <div ref={rootRef} onInputCapture={rememberFocus} className="min-h-screen">
    ...
    <div className="fixed bottom-4 left-1/2 z-[90] w-[calc(100%-24px)] max-w-md -translate-x-1/2 lg:bottom-6 lg:left-6 lg:w-auto lg:translate-x-0" dir="rtl"><div className="flex items-center gap-3 rounded-2xl border border-white/15 bg-[#111]/95 p-2.5 shadow-2xl backdrop-blur-xl"><button type="button" disabled={isSaving||!hasUnsavedChanges} onClick={saveChanges} className="flex grow items-center justify-center gap-2 rounded-xl bg-[#D51F2B] px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:bg-[#2a2a2a] disabled:text-gray-500 lg:grow-0"><Save className="h-4 w-4"/>{isSaving?'جارٍ الحفظ...':'حفظ التعديلات'}</button><div className="min-w-0 px-1 text-[11px]">{hasUnsavedChanges?<span className="flex items-center gap-1.5 text-amber-300"><AlertCircle className="h-3.5 w-3.5"/> توجد تعديلات غير محفوظة</span>:<span className="flex items-center gap-1.5 text-green-400"><CheckCircle2 className="h-3.5 w-3.5"/> {lastSavedAt?'تم الحفظ':'لا توجد تعديلات'}</span>}</div></div></div><div className="h-24 lg:h-0"/>
  </div>;
};
